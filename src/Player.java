...
public class Player {
    ...
    public boolean savePlaylistWithPaths(int index) {
        if (index >= 0 && index < playlists.size()) {
            Playlist playlist = playlists.get(index);
            try (PrintWriter out = new PrintWriter(new FileOutputStream(System.getProperty("user.home") + "/Desktop/playlist_paths" + index + ".csv"))) {
                for (Song song : playlist.getSongs()) {
                    out.printf("%s,%s,%d,%s\n", song.getTitle(), song.getArtist(), song.getDuration(), song.getFilePath());
                }
                return true;
            } catch (IOException e) {
                return false;
    ...
    public boolean loadPlaylist(int index, String playlistName) {
        File playlistFile = new File(System.getProperty("user.home") + "/Desktop/playlist_paths" + index + ".csv");
            ...
            try (BufferedReader reader = new BufferedReader(new FileReader(playlistFile))) {
                ...
                while ((line = reader.readLine()) != null) {
                    ...
                    if (songData.length == 4) {
                        ...
                        int duration = Integer.parseInt(songData[2]);
                        String filePath = songData[3];
                        Song song = new Song(title, artist, duration, filePath);
    ...
    public boolean addSongToPlaylist(int playlistIndex, String title, String artist, int duration, String filePath) {
